  ...
  return <main className="overflow-hidden">
    <header className="fixed inset-x-0 top-0 z-50 border-b border-navy/10 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
        <a href="#top" aria-label="Talent Nexus home"><Image src="/logo.png" alt="Talent Nexus" width={215} height={126} priority className="h-14 w-auto rounded bg-navy object-contain px-2" /></a>
    ...
    <section id="top" className="grid-lines relative min-h-screen pt-20">
      <div className="absolute right-[-10rem] top-28 h-96 w-96 rounded-full bg-cyan/20 blur-3xl" />
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl items-center px-5 py-20 lg:grid-cols-[1fr_.65fr] lg:px-8">
        ...
        <div className="relative hidden h-[34rem] lg:block">
          <div className="absolute inset-16 rounded-full border border-navy/15" /><div className="absolute inset-28 rounded-full border border-cyan/50" />
          <div className="absolute left-4 top-16 h-3 w-3 rounded-full bg-cyan shadow-[0_0_0_12px_rgba(22,220,197,.12)]" /><div className="absolute bottom-24 right-10 h-5 w-5 rounded-full bg-navy" />
          <div className="absolute left-1/2 top-1/2 w-52 -translate-x-1/2 -translate-y-1/2 rounded-3xl bg-navy p-7 text-white shadow-soft"><span className="text-xs tracking-[.18em] text-cyan">THE NEXUS</span><p className="mt-3 text-2xl font-bold">Talent meets opportunity.</p></div>
        </div>
      </div>
    </section>

    <section id="about" className="bg-mist py-24 sm:py-32"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid gap-12 lg:grid-cols-[.7fr_1.3fr]"><div><p className="text-xs font-bold tracking-[.24em] text-teal-600">{t.aboutLabel}</p><h2 className="mt-5 text-4xl font-bold tracking-tight sm:text-5xl">{t.aboutTitle}</h2></div><div><p className="max-w-3xl text-xl leading-9 text-ink/75 sm:text-2xl sm:leading-10">{t.about}</p><p className="mt-7 max-w-3xl text-base leading-8 text-ink/60">{t.aboutLead}</p></div></div><div className="mt-16 grid gap-5 md:grid-cols-3">{t.principles.map((item, i) => <div key={item[0]} className="border-t border-navy/15 pt-6"><span className="text-xs font-bold text-teal-600">0{i + 1}</span><h3 className="mt-3 text-lg font-bold">{item[0]}</h3><p className="mt-2 leading-7 text-ink/60">{item[1]}</p></div>)}</div><div className="mt-16 rounded-3xl bg-white p-7 shadow-soft sm:p-10"><p className="text-xs font-bold tracking-[.22em] text-teal-600">{t.sectorsLabel}</p><div className="mt-6 flex flex-wrap gap-3">{t.sectors.map(sector => <span key={sector} className="rounded-full border border-navy/10 bg-mist px-5 py-3 text-sm font-semibold">{sector}</span>)}</div></div></div></section>

    <section id="services" className="py-24 sm:py-32"><div className="mx-auto max-w-7xl px-5 lg:px-8"><p className="text-xs font-bold tracking-[.24em] text-teal-600">{t.servicesLabel}</p><div className="mt-5 grid items-end gap-6 lg:grid-cols-2"><h2 className="text-4xl font-bold tracking-tight sm:text-5xl">{t.servicesTitle}</h2><p className="max-w-xl leading-7 text-ink/60 lg:justify-self-end">{t.serviceNote}</p></div><div className="mt-14 grid gap-5 lg:grid-cols-3">{t.services.map((service, i) => { const Icon = icons[i]; return <article key={service[0]} className="group rounded-3xl border border-navy/10 bg-white p-8 transition duration-300 hover:-translate-y-2 hover:border-cyan hover:shadow-soft"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan/15 text-teal-700 transition group-hover:bg-cyan"><Icon size={23} /></div><span className="mt-10 block text-sm font-bold text-navy/35">0{i + 1}</span><h3 className="mt-3 text-2xl font-bold">{service[0]}</h3><p className="mt-4 leading-7 text-ink/65">{service[1]}</p></article>; })}</div></div></section>
